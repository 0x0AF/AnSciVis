...
GLuint loadShaders(std::string const &vs, std::string const &fs, const int task_nbr, const int enable_lighting, const int enable_shadowing, const int enable_opeacity_cor)
{
...
struct RaycasterConf
{
    ...
    glm::vec3 g_ambient_light_color = glm::vec3(0.5f, 0.5f, 0.5f);
    ...
    bool g_lighting_toggle = true;
    bool g_shadow_toggle = true;
...
struct Manipulator
{
    ...
    glm::mat4 matrix()
    {
        ...
        glm::vec2 m_delta = glm::vec2(velocity * (current_millis - last_millis), 0.0f);
...
bool read_volume(std::string &volume_string, RaycasterConf &conf, Volume_loader_raw &g_volume_loader, volume_data_type &g_volume_data, Cube &g_cube)
{
    ...
    conf.g_sampling_distance = 1.0f / glm::max(glm::max(conf.g_vol_dimensions.x, conf.g_vol_dimensions.y), conf.g_vol_dimensions.z);

    unsigned max_dim = std::max(std::max(conf.g_vol_dimensions.x, conf.g_vol_dimensions.y), conf.g_vol_dimensions.z);

    // calculating max volume bounds of volume (0.0 .. 1.0)
    conf.g_max_volume_bounds = glm::vec3(conf.g_vol_dimensions) / glm::vec3((float)max_dim);

    // loading volume file data
    g_volume_data = g_volume_loader.load_volume(conf.g_file_string);
    conf.g_channel_size = g_volume_loader.get_bit_per_channel(conf.g_file_string) / 8;
    ...
    conf.g_volume_texture = createTexture3D(conf.g_vol_dimensions.x, conf.g_vol_dimensions.y, conf.g_vol_dimensions.z, conf.g_channel_size, conf.g_channel_count, (char *)&g_volume_data[0]);
...
void UpdateImGui(RaycasterConf &conf, Window &g_win)
{
    ...
    io.MouseDown[0] = conf.mousePressed[0] || SDL_GetMouseState(NULL, NULL) & SDL_BUTTON(SDL_BUTTON_LEFT);
    io.MouseDown[1] = conf.mousePressed[1] || SDL_GetMouseState(NULL, NULL) & SDL_BUTTON(SDL_BUTTON_RIGHT);

    ImGui_ImplSdlGL3_NewFrame(g_win.getSDLwindow());
}

void showGUI(RaycasterConf &conf, Window &g_win, Transfer_function &g_transfer_fun, Volume_loader_raw &g_volume_loader, volume_data_type &g_volume_data, Cube &g_cube)
{
    ...
    if(ImGui::CollapsingHeader("Task", 0, true, true))
    {
        if(ImGui::TreeNode("Introduction"))
        {
            ...
            ImGui::RadioButton("Average Intensity Projection", &conf.g_task_chosen, 11);
            ImGui::TreePop();
        }
        ...
        if(ImGui::TreeNode("Iso Surface Rendering"))
        {
            ...
            ImGui::SliderFloat("Iso Value", &conf.g_iso_value, 0.0f, 1.0f, "%.8f", 1.0f);
            ImGui::TreePop();
        }
        ...
        ImGui::SameLine();
        conf.g_task_chosen == 31 || conf.g_task_chosen == 12 || conf.g_task_chosen == 13 ? ImGui::Text("Enable Lighting") : ImGui::TextColored(ImVec4(0.2f, 0.2f, 0.2f, 0.5f), "Enable Lighting");

        conf.g_reload_shader ^= ImGui::Checkbox("2", &conf.g_shadow_toggle);
        ImGui::SameLine();
        conf.g_task_chosen == 31 || conf.g_task_chosen == 12 || conf.g_task_chosen == 13 ? ImGui::Text("Enable Shadows") : ImGui::TextColored(ImVec4(0.2f, 0.2f, 0.2f, 0.5f), "Enable Shadows");

        conf.g_reload_shader ^= ImGui::Checkbox("3", &conf.g_opacity_correction_toggle);
        ImGui::SameLine();
        conf.g_task_chosen == 31 ? ImGui::Text("Opacity Correction") : ImGui::TextColored(ImVec4(0.2f, 0.2f, 0.2f, 0.5f), "Opacity Correction");
    ...
    if(ImGui::CollapsingHeader("Load Volumes", 0, true, false))
    {
        ...
        if(load_volume_1)
        {
            ...
            read_volume(conf.g_file_string, conf, g_volume_loader, g_volume_data, g_cube);
        }
        if(load_volume_2)
        {
    ...
    if(ImGui::CollapsingHeader("Lighting Settings"))
    {
        ...
        ImGui::SliderFloat("Reflection Coefficient kd", &conf.g_ref_coef, 0.0f, 20.0f, "%.5f", 1.0f);
    }
    ...
    if(ImGui::CollapsingHeader("Quality Settings"))
    {
        ...
        ImGui::SliderFloat("sampling factor", &conf.g_sampling_distance_fact, 0.1f, 10.0f, "%.5f", 4.0f);
        ImGui::SliderFloat("sampling factor move", &conf.g_sampling_distance_fact_move, 0.1f, 10.0f, "%.5f", 4.0f);
        ImGui::SliderFloat("reference sampling factor", &conf.g_sampling_distance_fact_ref, 0.1f, 10.0f, "%.5f", 4.0f);
    }
    ...
    ImGui::End();
    conf.g_show_transfer_function = ImGui::Begin("Transfer Function Window", &conf.g_show_transfer_function_in_window, ImVec2(300, 500));

    conf.g_transfer_function_pos.x = ImGui::GetItemRectMin().x;
    conf.g_transfer_function_pos.y = ImGui::GetItemRectMin().y;

    conf.g_transfer_function_size.x = ImGui::GetItemRectMax().x - ImGui::GetItemRectMin().x;
    conf.g_transfer_function_size.y = ImGui::GetItemRectMax().y - ImGui::GetItemRectMin().y;
    ...
    conf.g_transfer_function_pos.x = ImGui::GetItemRectMin().x;
    conf.g_transfer_function_pos.y = ImGui::GetIO().DisplaySize.y - ImGui::GetItemRectMin().y - 70;
    ...
    if(ImGui::CollapsingHeader("Manipulate Values"))
    {
        ...
        for(Transfer_function::container_type::iterator c = con.begin(); c != con.end(); ++c)
        {
            ...
            is << i;
            change_value ^= ImGui::SliderInt(is.str().c_str(), &g_c_data_value[i], 0, 255);
            ImGui::SameLine();
...
int main(int argc, char **argv)
{
    ...
    bool check = read_volume(conf.g_file_string, conf, g_volume_loader, g_volume_data, g_cube);

    glActiveTexture(GL_TEXTURE1);
    conf.g_transfer_texture = createTexture2D(255u, 1u, (char *)&g_transfer_fun.get_RGBA_transfer_function_buffer()[0]);

    try
    {
        g_volume_program = loadShaders(g_file_vertex_shader, g_file_fragment_shader, conf.g_task_chosen, conf.g_lighting_toggle, conf.g_shadow_toggle, conf.g_opacity_correction_toggle);
    }
    ...
    while(!g_win.shouldClose)
    {
        ...
        if(!conf.first_frame > 0.0)
        {
            ...
            if(ImGui::IsKeyPressed(SDL_SCANCODE_MINUS) || ImGui::IsKeyPressed(SDL_SCANCODE_KP_MINUS))
            {
                conf.g_iso_value -= 0.002f;
                conf.g_iso_value = std::max(conf.g_iso_value, 0.0f);
            }

            if(ImGui::IsKeyPressed(SDL_SCANCODE_EQUALS) || ImGui::IsKeyPressed(SDL_SCANCODE_KP_PLUS))
            {
                conf.g_iso_value += 0.002f;
            ...
            if(ImGui::IsMouseDown(SDL_BUTTON_LEFT) || ImGui::IsMouseDown(SDL_BUTTON_MIDDLE) || ImGui::IsMouseDown(SDL_BUTTON_RIGHT))
            {
        ...
        if(conf.g_reload_shader)
        {
            ...
            try
            {
                ...
                newProgram = loadShaders(g_file_vertex_shader, g_file_fragment_shader, conf.g_task_chosen, conf.g_lighting_toggle, conf.g_shadow_toggle, conf.g_opacity_correction_toggle);
        ...
        if(conf.g_transfer_dirty && !conf.first_frame)
        {
            ...
            glActiveTexture(GL_TEXTURE1);
            updateTexture2D(conf.g_transfer_texture, 255u, 1u, (char *)&g_transfer_fun.get_RGBA_transfer_function_buffer()[0]);
        }
        ...
        glClearColor(conf.g_background_color.x, conf.g_background_color.y, conf.g_background_color.z, 1.0);
        ...
        glm::vec3 translate_rot = conf.g_max_volume_bounds * glm::vec3(-0.5f, -0.5f, -0.5f);
        glm::vec3 translate_pos = conf.g_max_volume_bounds * glm::vec3(+0.5f, -0.0f, -0.0f);
        ...
        glm::mat4 model_view = view * glm::translate(translate_pos) * turntable_matrix * glm::rotate(0.5f * float(M_PI), glm::vec3(0.0f, 1.0f, 0.0f)) *
                               glm::rotate(0.5f * float(M_PI), glm::vec3(1.0f, 0.0f, 0.0f)) * glm::translate(translate_rot);
        ...
        glUniform1f(glGetUniformLocation(g_volume_program, "sampling_distance"), conf.g_sampling_distance * sampling_fact);
        glUniform1f(glGetUniformLocation(g_volume_program, "sampling_distance_ref"), conf.g_sampling_distance_fact_ref);
        glUniform1f(glGetUniformLocation(g_volume_program, "iso_value"), conf.g_iso_value);
        glUniform3fv(glGetUniformLocation(g_volume_program, "max_bounds"), 1, glm::value_ptr(conf.g_max_volume_bounds));
        glUniform3iv(glGetUniformLocation(g_volume_program, "volume_dimensions"), 1, glm::value_ptr(conf.g_vol_dimensions));
        glUniform3fv(glGetUniformLocation(g_volume_program, "light_position"), 1, glm::value_ptr(conf.g_light_pos));
        glUniform3fv(glGetUniformLocation(g_volume_program, "light_ambient_color"), 1, glm::value_ptr(conf.g_ambient_light_color));
        glUniform3fv(glGetUniformLocation(g_volume_program, "light_diffuse_color"), 1, glm::value_ptr(conf.g_diffuse_light_color));
        glUniform3fv(glGetUniformLocation(g_volume_program, "light_specular_color"), 1, glm::value_ptr(conf.g_specula_light_color));
        glUniform1f(glGetUniformLocation(g_volume_program, "light_ref_coef"), conf.g_ref_coef);
        ...
        while(SDL_PollEvent(&event))
        {
            ...
            case SDL_MOUSEMOTION:
                g_win.cursorPositionCallback(g_win.getSDLwindow(), event.motion.x, event.motion.y);
                break;
            case SDL_MOUSEBUTTONDOWN:
                conf.mousePressed[0] = true;
                g_win.mouseButtonCallback(g_win.getSDLwindow(), event.button.button, SDL_PRESSED, 0);
                break;
            case SDL_MOUSEBUTTONUP:
                conf.mousePressed[0] = false;
                g_win.mouseButtonCallback(g_win.getSDLwindow(), event.button.button, SDL_RELEASED, 0);
                break;
            case SDL_KEYDOWN:
                g_win.keyCallback(g_win.getSDLwindow(), event.key.keysym.scancode, event.key.keysym.scancode, SDL_PRESSED, 0);
                break;
            case SDL_KEYUP:
                g_win.keyCallback(g_win.getSDLwindow(), event.key.keysym.scancode, event.key.keysym.scancode, SDL_RELEASED, 0);
                break;
            default:
                break;
            }
        ...
        showGUI(conf, g_win, g_transfer_fun, g_volume_loader, g_volume_data, g_cube);
        ...
        if(conf.g_show_transfer_function)
            g_transfer_fun.draw_texture(conf.g_transfer_function_pos, conf.g_transfer_function_size, conf.g_transfer_texture);
