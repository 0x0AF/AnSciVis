...
Transfer_function::Transfer_function()
    : m_piecewise_container(), m_program_id(0),
      // m_vao(0),
      m_plane(), m_dirty(true)
{
    m_program_id = createProgram(vertex_shader, fragment_shader);
}

void Transfer_function::add(float data_value, glm::vec4 color) { add((unsigned)(data_value * 255.0), color); }
void Transfer_function::add(unsigned data_value, glm::vec4 color)
{
...
void Transfer_function::remove(unsigned data_value)
{
    helper::clamp(data_value, 0u, 255u);
...
void Transfer_function::reset()
{
    m_piecewise_container.clear();
    m_dirty = true;
}

void Transfer_function::draw_texture(const glm::vec2 &tf_pos, const glm::vec2 &tf_size, const GLuint &texture) const
{
    ...
    glUniformMatrix4fv(glGetUniformLocation(m_program_id, "Projection"), 1, GL_FALSE, &ortho_projection[0][0]);
    glUniformMatrix4fv(glGetUniformLocation(m_program_id, "Modelview"), 1, GL_FALSE, glm::value_ptr(view));
